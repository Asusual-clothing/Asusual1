...
app.post("/delete_product/:id", async (req, res) => {
  try {
    ...
    console.log(
      "Attempting to delete the following Cloudinary public_ids:",
      idsToDelete
    );
...
app.post("/add-to-cart", async (req, res) => {
  ...
  try {
    ...
    const redirectUrl =
      action === "buy" ? "/cart" : `/product_details/${productId}`;
    res.send(`
           <script>
...
app.post("/create", async (req, res) => {
  try {
    const { code, discountType, discountValue, expiryDate, active, useonce } =
      req.body;
...
app.post(
  ...
  async (req, res) => {
    try {
      const { name, description, price, brand, bestseller, color, category } =
        req.body;
...
app.get("/cart", async (req, res) => {
  try {
    ...
    const deliveryCostDoc = await DeliveryCost.findOne().sort({
      updatedAt: -1,
    });
    const deliveryCost = deliveryCostDoc ? deliveryCostDoc.cost : 5.0; // Default to 5.00 if not found
    ...
    if (cart.appliedCoupon?.useonce) {
      ...
      if (user.useoncecoupon.includes(cart.appliedCoupon.code)) {
        return res.status(400).json({
          success: false,
          message:
            "This coupon can only be used once per user and has already been used",
        });
      }
    }
...
app.get("/orders/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .render("error", { message: "Server error fetching order details" });
  }
});
...
app.post("/orders/create", async (req, res) => {
  try {
    ...
    // ✅ Return the customer name and email in response
    res.json({
      success: true,
      order: newOrder,
      customer: {
...
app.get("/payment/verify", async (req, res) => {
  try {
    ...
    // - Fetch transaction details from Cashfree API
    // - Verify signature
